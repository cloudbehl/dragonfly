...
def create_file(output_list):
	if not os.path.exists(os.getcwd()+"/"+output_list[2]+"."+output_list[1]):
		if(output_list[1]=="html"):
			...
			print("Do you really want to create this file: %s.%s"% (output_list[2],output_list[1]))
	...
	while(1):
		...
		if(str(listen_output)!="Could not understand audio"):
			...
			if((output_list[0]=="mkdir" or output_list[0]=="make")and len(output_list)>=2):
				...
				move_back()
			elif((output_list[0]=="cd" and len(output_list)>=2) or ((output_list[0]=="change" and output_list[1]=="directory") and len(output_list)>=3)):
